...
class Settings(BaseSettings):
    ...
    LLM_MODEL: str = "gemma3:4b"  # Used by llm_engine.py (OpenAI-compat endpoint)

    # Ollama settings — ollama_client.py uses OLLAMA_MODEL, llm_engine.py uses LLM_MODEL
    # Keep both in sync to avoid confusion. Set OLLAMA_MODEL = LLM_MODEL in .env if using one model.
    OLLAMA_BASE_URL: str = "http://localhost:11434"
    OLLAMA_MODEL: str = "gemma3:4b"  # Must match LLM_MODEL for consistent behavior
    OLLAMA_TIMEOUT: int = 600  # seconds per request (generous for large JSON output)
    ...
    WAN_NUM_FRAMES: int = 17          # ~3.4 seconds at 24fps
    WAN_NUM_INFERENCE_STEPS: int = 4  # Diffusion steps (lower = faster)
    ...
    DEFAULT_WIDTH: int = 480         # Default generated video width
    DEFAULT_HEIGHT: int = 832         # Default generated video height
