...
def _normalize_dialogue_list(dialogue_list: List[Dict]) -> List[Dict]:
    """
    Normalize dialogue from LLM format to TTS pipeline format.
    LLM may produce: {"character_name": "...", "dialogue_text": "..."}
    TTS expects:     {"speaker": "...", "line": "..."}
    Also handles nested lists or other malformed structures.
    """
    normalized = []
    ...
    for item in dialogue_list:
        ...
        speaker = item.get("speaker") or item.get("character_name") or item.get("name") or "Narrator"
        line = (
            item.get("line")
        ...
        if line.strip():
            normalized.append({"speaker": str(speaker), "line": line.strip()})
    ...
    async def generate_episode_pipeline(
        ...
        try:
            ...
            for idx, scene_info in enumerate(scenes_data, 1):
                ...
                # LLMs occasionally misname keys or leave them empty. We need robust fallbacks.
                motion_prompt = scene_info.get("video_motion_prompt") or scene_info.get("video_ motion_ prompt") or scene_info.get("visual_description") or "Cinematic camera movement"
                img_prompt = scene_info.get("image_prompt") or scene_info.get("visual_description") or "Cinematic scene"
                
                # Normalize dialogue keys from LLM format (character_name/dialogue_text)
                # to TTS pipeline format (speaker/line)
                raw_dialogue = scene_info.get("dialogue", [])
                normalized_dialogue = _normalize_dialogue_list(raw_dialogue)
                if not normalized_dialogue:
                    logger.warning(
                        "Scene %d has no valid dialogue after normalization. Raw: %s",
                        idx, raw_dialogue
                    )
                ...
                scene_obj = Scene(
                    ...
                    time_of_day="NIGHT" if "NIGHT" in scene_info.get("location", "").upper() else "DAY",
                    visual_description=scene_info.get("visual_description", ""),
                    image_prompt=img_prompt,
                    video_motion_prompt=motion_prompt,
                    dialogue_script=normalized_dialogue,
                ...
                scene_obj.duration_seconds = max(scene_dur, 3.0)  # Ensure at least 3 seconds for video
                total_duration += scene_obj.duration_seconds
                db.commit()

                # Generate Video Motion (CogVideoX / Wan) using the exact audio duration
            ...
            # Set thumbnail from the first scene's generated image
            first_scene = db.query(Scene).filter(Scene.episode_id == episode.id).order_by(Scene.scene_number).first()
            if first_scene and first_scene.image_url:
                episode.thumbnail_url = first_scene.image_url
