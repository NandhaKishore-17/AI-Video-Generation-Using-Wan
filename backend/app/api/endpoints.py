...
from app.engines.llm_engine import LLMUnavailableError
...
async def create_universe(payload: UniverseCreate, db: Session = Depends(get_db)):
    ...
    All content is generated by the LLM — no hardcoded fallbacks.
    """
    try:
        ...
        raise HTTPException(
            status_code=500,
            detail={"error": "LLM unavailable", "reason": str(exc)}
        )
...
async def generate_episode(payload: EpisodeGenerateRequest, background_tasks: BackgroundTasks, db: Session = Depends(get_db)):
    ...
    All screenplay content is generated by the LLM — no hardcoded fallbacks.
    """
    universe = db.query(Universe).filter(Universe.id == payload.universe_id).first()
    if not universe:
        raise HTTPException(status_code=404, detail="Universe not found")
