...
export const UniverseManager: React.FC<UniverseManagerProps> = ({ activeUniverseId, onSelectUniverse, onUniversesChanged }) => {
  ...
  const handleCreate = async (e: React.FormEvent) => {
    ...
    try {
      ...
      setError(err?.message || 'Failed to create universe. Make sure Ollama is running with the correct model.');
    } finally {
      setLoading(false);
    }
  };
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8">
        ...
        <GlassCard className="lg:col-span-1 space-y-4">
          ...
          <form onSubmit={handleCreate} className="space-y-4 text-xs">
            ...
            {error && (
              <div className="p-3 rounded-lg bg-rose-950/80 border border-rose-700 text-xs font-mono text-rose-300 space-y-1">
                <span className="font-bold block text-rose-400">⚠ UNIVERSE GENERATION FAILED</span>
                <p>{error}</p>
                <button
                  type="button"
                  onClick={() => setError(null)}
                  className="text-[10px] text-rose-500 hover:text-rose-300 underline mt-1"
                >
                  Dismiss
                </button>
              </div>
