    ...
    <div className="space-y-8">
      ...
      <GlassCard glow className="max-w-2xl">
        <form onSubmit={handleSave} className="space-y-5 text-xs font-mono">
          ...
          <div>
            <label className="block text-slate-300 font-bold mb-1">VIDEO MOTION SYNTHESIZER</label>
            <select
              value={videoModel}
              onChange={(e) => setVideoModel(e.target.value)}
              className="w-full bg-slate-900 border border-slate-800 rounded-xl px-3 py-2.5 text-white font-bold focus:outline-none focus:border-cyan-500 cursor-pointer"
            >
              <option value="Wan-AI/Wan2.1-T2V-1.3B (CogVideoX)">Wan-AI/Wan2.1-T2V-1.3B (CogVideoX)</option>
              <option value="Wan-AI/Wan2.2-TI2V-5B (Local/Cloud)">Wan-AI/Wan2.2-TI2V-5B (Local/Cloud)</option>
              <option value="Mock (Fast/Testing)">Mock (Fast/Testing)</option>
            </select>
          </div>
